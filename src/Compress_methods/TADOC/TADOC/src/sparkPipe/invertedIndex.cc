...
cin>>threadID;
  ...
  indexvec = (vector<int>*) new vector<int>[words];
  ...
  if(threadID!=0)
    indexvec[split[offset-2]].push_back(offset-1);
  ...
  for(int i=0; i<splitNumSepa[threadID]; i++){
    ...
    for(vector<int>::iterator it=wordVecWord.begin(); it!=wordVecWord.end(); it++){
      indexvec[*it].push_back(i+offset-1);
      count4[*it]=0;
    }
  }
  ...
  for(int i=0; i<words; i++){
    for(vector<int>::iterator it=indexvec[i].begin(); it!=indexvec[i].end(); it++)
      cout<<i<<" "<<*it<<endl;
  }
