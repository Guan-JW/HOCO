...
int Query_delete(int searchFile, ulong searchStart_charoff, int extract_length) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < query_size; i++) {
        Query_delete(query_file_indexes[i], query_insert_offsets[i], searchLen);
        cnt++;
    }
    time9 = clock();
    cout << "IO(s) : " << (double)(t2 - t1) / CLOCKS_PER_SEC << endl;
    cout << "DELETE TIME : " << (double)(time9 - time8) / CLOCKS_PER_SEC << "s" << endl;
