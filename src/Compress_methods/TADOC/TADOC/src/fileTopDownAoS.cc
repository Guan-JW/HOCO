...
bool hasThisBit(int rule, int file){
  ...
  if(!(fileIdx1st[rule][firstLyerShift6] & 1ul<<((firstLyer)&0x3f))){//none
    fileIdx1st[rule][firstLyerShift6] = fileIdx1st[rule][firstLyerShift6]|(1ul<<((firstLyer)&0x3f));

    filePtr[rule][firstLyer]=(unsigned long*)malloc(sizeof(unsigned long)*(ZwiftSpaceShift6));
    memset(filePtr[rule][firstLyer],0,(sizeof(unsigned long)*(ZwiftSpaceShift6)));
    filePtr[rule][(firstLyer)][(file&0x3ff)>>6] = filePtr[rule][(firstLyer)][(file&0x3ff)>>6] | (1ul<<((file)&0x3ff));
    return false;
  }
  else{
    if(!(filePtr[rule][(firstLyer)][(file&0x3ff)>>6] & (1ul<<((file)&0x3ff)))){
      filePtr[rule][(firstLyer)][(file&0x3ff)>>6] = filePtr[rule][(firstLyer)][(file&0x3ff)>>6] | (1ul<<((file)&0x3ff));
      return false;
    }
    else  return true;
...
finput>>fileid;
 ...
 fileIdx1st = new unsigned long*[rules];
 filePtr = new unsigned long**[rules];


  for(int i=1; i<rules; i++){
    fileIdx1st[i] = (unsigned long*)malloc(sizeof(unsigned long)*size1stUL  );
    memset(fileIdx1st[i],0,(sizeof(unsigned long)*size1stUL));
    filePtr[i] = (unsigned long**)malloc(sizeof(unsigned long*)* size1st );
    memset(filePtr[i],0,(sizeof(unsigned long*)*size1st));
  }
  ...
  for(int i=0; i<splitNum; i++){
    ...
    for(vector<int>::iterator p=st.begin(); p!=st.end(); p++){
      ...
      if(*p<words){
        ...
        unsigned long firstLyerShift6 = (firstLyer>>6);
        int ZwiftSpaceShift6 = (ZwiftSpace>>6);
        fileIdx1st[(*p)-words][firstLyerShift6 ] = fileIdx1st[(*p)-words][firstLyerShift6] | (1ul<<((firstLyer)&0x3f));
        
        if(filePtr[(*p)-words][firstLyer]==NULL){
          filePtr[(*p)-words][firstLyer]=(unsigned long*)malloc(sizeof(unsigned long)*(ZwiftSpaceShift6));
          memset(filePtr[(*p)-words][firstLyer],0,(sizeof(unsigned long)*(ZwiftSpaceShift6)));
        }
       
        filePtr[(*p)-words][(firstLyer)][i3ff6] = filePtr[(*p)-words][(firstLyer)][i3ff6] | i13ff;
