...
int main(int argc, char** argv){
  ...
  sprintf(relationDir,"%s",argv[1]);
    ...
    for(int j=0; j<totalJ; j++){
      ...
      all.erase(searchStart, searchLen);
      fileLength -= searchLen;
      cnt ++;
      if (fileLength <= searchLen)  break;
    }
    double time7=timestamp();

    cout << "IO(s) : " << (double)(t2 - t1) / CLOCKS_PER_SEC << endl;
    cout<<"AVGLatency(s): "<<(time7-time6)/cnt<<endl;
    cout<<"AVGLatency(us): "<<(time7-time6)/cnt*1000000<<endl;
    cout<<"totalTime(s): "<<(time7-time6)<<endl;
    cout<<"Throughput(op/s): "<<(cnt)/(time7-time6)<<endl;
